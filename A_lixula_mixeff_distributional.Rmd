...
Making a dataset containing only the data pertaining to *A. lixula*.

```{r import data}
Al_Al_df <-  read_delim("larval_morphology.csv", delim = ",",
                  col_types = "fffnfiffffiniif")
Al_Al_df = drop_na(Al_Al_df, length)
# make Al_df$larva by concatenating Al_df$species with Al_df$larva 
Al_Al_df$larva <- as.factor(paste0(Al_Al_df$species, Al_Al_df$larva))
Al_Al_df <- Al_Al_df[Al_Al_df$species == "Al",]
#ensure correct order for levels
Al_Al_df<-Al_Al_df %>% mutate(lit = factor(lit, levels = c("DD", "LD", "LL")))
Al_Al_df<-Al_Al_df %>% mutate(rod = factor(rod, levels = c("BR", "PO", "ALA")))
head(Al_Al_df)
```

```{r}
# Weird - need to fix
Al_Al_df[Al_Al_df$length < 0,]
```

```{r  }
Al_Al_df <- Al_Al_df[Al_Al_df$length > 0,]
Al_Al_df <- Al_Al_df[! is.na(Al_Al_df$length),]
print(paste0('There are ', dim(Al_Al_df)[1], ' measures from ', length(unique(Al_Al_df$larva)), ' individual larvae.'))
```
...
```{r data summary}
Al_Al_df %>% group_by(species,dpf, lit, rod, fed, lar_ml) %>%
...
grow much shorter arms when food is abundant to save maternal storages;
